...
def doit(args):
    ...
    if test.lower().startswith("allchars"):
        ...
        lowhamzas = list(filter(lambda x: x in builder.uids(), (0x0647, 0x064A, 0x06C1, 0X06D5)))
        if len(lowhamzas):
            ftml.startTestGroup('Low-hamza combinations')
            for base in lowhamzas:
...
#                                builder.render([uid1, mb, 0x0627, ma, ma, mb], ftml, addBreaks=False, rtl=True, dualJoinMode=1)
    ...
    if test.lower().startswith('feature-lang'):
        ...
        tests = filter(lambda x : x[0] in builder.features, (
            ...
            ('cv84', {'sd': 1, 'ur': 1, 'ku': 1, 'rhg': 1, 'wo': 1, 'ky': 1}, (0x060C,), 'Comma alternates'),
